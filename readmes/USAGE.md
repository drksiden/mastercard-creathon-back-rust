...
Система преобразует вопросы на естественном языке в SQL-запросы с помощью LLM через библиотеку `rig-core` (поддерживает Ollama/OpenAI/Anthropic и другие), выполняет их в PostgreSQL и возвращает результаты.
...
2. Backend отправляет вопрос в LLM через rig-core (Ollama/OpenAI)
...
```json
{
  ...
  "sql": "SELECT merchant_id, SUM(transaction_amount_kzt) as total_volume FROM transactions WHERE transaction_type = 'POS' GROUP BY merchant_id ORDER BY total_volume DESC LIMIT 5;",
  "data": [
...
- "Транзакции в категории 'Grocery & Food Markets'"
- "Транзакции в категории 'Electronics & Software'"
...
### По странам:
- "Транзакции в Казахстане (KAZ)"
- "Транзакции в США (USA)"
- "Сравнение транзакций по странам"

### По способам оплаты:
- "Транзакции через Contactless"
- "ECOM транзакции"
- "QR_Code транзакции"
- "Swipe транзакции"
...
- "Дневной объем транзакций в категории 'Dining & Restaurants' за последние 30 дней"
- "POS транзакции в валюте USD за последний месяц"
- "Contactless транзакции в категории 'Grocery & Food Markets'"
